...
- Web and Electron pin `@openchamber/opencode-container-workspace` to immutable plugin commit `fa8eefd9b6c85be37757c796a98e301c35e52417`. That commit carries the cleanup, snapshot-source, provider-readiness, and granted-egress fixes found during live Windows Docker validation, OpenCode 1.18.12, the refusal to snapshot a home directory or a filesystem root, and an explicit access list for workspace state on Windows, where the declared file modes are not implemented; it is the squashed merge of plugin PR #9 on the default branch, with plugin test, both image, docker-live, and kubernetes-live jobs green at that commit.
