...
- Web and Electron pin `@openchamber/opencode-container-workspace` to immutable plugin commit `b263b90632c7f2c1191eae288057b89de2c96f22`. That commit carries the cleanup, snapshot-source, provider-readiness, and granted-egress fixes found during live Windows Docker validation, OpenCode 1.18.12, the refusal to snapshot a home directory or a filesystem root, and an explicit access list for workspace state on Windows, where the declared file modes are not implemented; it is plugin PR #10 on the default branch, which carries PR #9 with test fixtures that no longer name a real machine, and PR #11, which establishes Kubernetes permissions from one listing rather than twenty-six process spawns on the default branch, with plugin test, both image, docker-live, and kubernetes-live jobs green at that commit.
...
- The runtime image Dockerfile installs OpenCode `1.18.12`, but that applies only to images built from it. The published images currently pinned as defaults were built with OpenCode `1.18.4`, so a workspace today runs a `1.18.4` CLI against a `1.18.12` host SDK. That gap is sound rather than accidental: the workspace surface — the v2 client, the generated SDK, and the generated types — is identical across `1.18.4`, `1.18.8`, and `1.18.12`. It closes when new images are published and their digests replace the defaults; until then this document must not describe the running CLI as `1.18.12`.
