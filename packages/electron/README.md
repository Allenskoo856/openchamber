...
6. `package.mjs` to verify the staged plugin, run `electron-builder`, stage the rebuilt `better-sqlite3` binary omitted by Electron Builder's Bun dependency collector, and verify the plugin in every unpacked packaged app.
...
Windows packaging needs NSIS support through `electron-builder`. Local builds remain unsigned when signing env is absent, but production release jobs set `OPENCHAMBER_REQUIRE_WINDOWS_SIGNING=1` and require `CSC_LINK` (or `WINDOWS_CSC_LINK`) plus `CSC_KEY_PASSWORD`; the workflow verifies a valid Authenticode signature and timestamp before upload. Windows updates use `latest.yml` for x64 and the `latest-arm64.yml` channel for ARM64 so each installation resolves an architecture-matching installer.
...
Release jobs also run `bun run smoke:packaged` against the unpacked application produced during packaging. It launches the real packaged executable and waits for the in-process server-ready signal followed by application renderer navigation readiness. The smoke requires packaged mode, `--openchamber-packaged-smoke`, and both `OPENCHAMBER_PACKAGED_SMOKE=1` and `OPENCHAMBER_PACKAGED_SMOKE_DIR`; normal launches do not expose this path or any IPC. It checks the bundled workspace plugin and OpenCode CLI resources before launch.

Physical desktop tests are separate from release CI and run only through the manually dispatched `Secure Workspace Physical Tests` workflow after protected `desktop-windows` or `desktop-linux` approval. Dedicated self-hosted runners install or launch an exact same-repository Actions artifact and run packaged smoke with `OPENCHAMBER_PACKAGED_SMOKE_WORKSPACE=1`. The Linux runner must provide FUSE/libfuse2 and an unlocked graphical session; its AppImage is executed directly rather than through extraction fallback. Smoke mode uses an isolated temporary OpenChamber/Chromium profile and independent step-up password, configures exact digest-pinned Docker runtime and gateway images, starts a routed Secure Workspace session, requires provider cleanup with no remaining resources, writes commit-bound test output without machine identifiers, and removes the temporary profile. Windows also attempts silent uninstall. This automated smoke is not release certification and does not yet replace the interactive apply procedure in `docs/SECURE_WORKSPACES_PHYSICAL_TEST_SETUP.md`.
