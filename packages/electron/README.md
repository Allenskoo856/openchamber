...
Native desktop validation uses a guided OpenChamber session in the candidate checkout on the target Windows, Linux, or macOS host; it does not require a self-hosted GitHub runner. The assistant builds and launches the exact native package with isolated OpenChamber, Chromium, OpenCode, Docker, and Kubernetes profiles, while the operator handles UAC, reboot, system dialogs, and interactive review/apply. `OPENCHAMBER_PACKAGED_SMOKE_WORKSPACE=1` remains a useful local Docker lifecycle and cleanup check, but automated smoke is not physical platform or interactive-apply evidence. Windows covers Docker Desktop and focused Kubernetes integration; Linux covers a direct native AppImage, full Docker, and disposable `kind`. See `docs/SECURE_WORKSPACES_PHYSICAL_TEST_SETUP.md` for the complete matrix and cleanup rules.
