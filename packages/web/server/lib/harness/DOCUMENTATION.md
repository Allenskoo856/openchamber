...
| Sticky session bindings | `session-bindings.js` (in-memory Map, v1) |
| Prompt / abort dispatch | `router.js` |
...
- Claude Code engine traffic uses OpenChamber routes `/api/harness/*` via
  `runtimeFetch` (UI slice lands later).
...
- `harnessId` on a binding is sticky; engine switch requires a new session
  (handoff lands in a later slice).
...
`packages/web/server/index.js` wires both. Tests may inject a mocked
`router` / `detectAll` / `detectOne`.
...
- `startClaudeQuery({ prompt, cwd, model, resume, permissionMode, env })`
...
- `packages/ui/src/lib/harness/client.ts` — `harnessPrompt` / `harnessAbort` via `runtimeFetch`
...
- Permissions bridge (`canUseTool` / `/api/harness/permission/reply`)
- Handoff duplicate + billing notice
- Durable binding persistence across process restarts
