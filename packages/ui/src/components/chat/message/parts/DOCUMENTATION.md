...
- Running bash output falls back to `state.metadata.output` until canonical `state.output` arrives. Its fixed-height output viewport follows new output until the user scrolls up, then resumes following when the user returns to the bottom. Live output appends as plain text without worker highlighting; finalized output receives the normal one-time highlighted rendering.
