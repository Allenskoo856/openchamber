...
- **Flat display sections**: `useSessionSidebarSections` still builds per-worktree `projectSections` as the data layer (bootstrap demand planning, ownership), and additionally derives `flatSectionsForRender` — one merged non-archived group per project (`id: 'flat'`, `folderScopes` listing every contributing scope). Rendering consumes only the flat sections; demand planning consumes `projectSections`.
