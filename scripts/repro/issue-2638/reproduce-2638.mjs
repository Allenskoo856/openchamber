...
try {
  ...
  if (BASELINE) {
    ...
    check('NEW server event is NOT delivered to the UI (stale UI)', deliveredAfter === 0,
      `delivered=${deliveredAfter}, total hub events=${received.length}`);
  }
  ...
  // 9. Orphan scenario only: show the hub is pinned to the OLD server —
  //    events emitted by the old (zombie) server still reach the UI.
  if (!BASELINE) {
    await emitEvent(p1, 'evt-zombie-old-server');
    await waitFor(() => received.some((r) => r.eventId === 'evt-zombie-old-server'), 5000, 'zombie event delivery');
    check('OLD zombie server events still reach the UI (hub pinned to stale upstream)', received.some((r) => r.eventId === 'evt-zombie-old-server'));
  }
